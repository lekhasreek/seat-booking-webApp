            ...
            */}
            ...
            <BookingModal
              ...
              onBook={async (bookingData) => {
                try {
                  ...
                  if ((!showBooking || !showBooking.seatId) && selectedSeatsForBooking.length > 0) {
                    ...
                    if (showBooking.isEdit && showBooking.bookingId) {
                      // Edit: merge new timeslots with existing booking timeslots and update
                      try {
                        // Retrieve existing booking object from bookingDetails prop or map
                        let existingBooking = showBooking.bookingDetails;
                        if (!existingBooking) {
                          // fallback: try to find from bookedSeatsMap
                          const label = currentSeatLabel;
                          existingBooking = Object.values(bookedSeatsMap[selectedDate]?.[label] || {})[0];
                        }
                        let existingTimes = [];
                        if (existingBooking && existingBooking.Timeslot) {
                          try {
                            existingTimes = typeof existingBooking.Timeslot === 'string' ? JSON.parse(existingBooking.Timeslot).timeslot : existingBooking.Timeslot.timeslot;
                          } catch (e) { existingTimes = []; }
                        }
                        const merged = [...existingTimes, ...bookingData.timeslot.timeslot];
                        const seen = new Set();
                        const deduped = [];
                        ...
                        const timeslotJson = JSON.stringify({ timeslot: deduped });
                        await editBooking({
                          Booking_id: showBooking.bookingId,
                          Seat_id: seatUUID,
                        ...
                        toast.error('Failed to edit booking: ' + (err?.message || err));
                        return;
                      }
                    } else {
                      // New booking: create a single booking with all timeslots
                      await insertBooking({
                        created_at: selectedDate,
                        Seat_id: seatUUID,
                        Timeslot: JSON.stringify({ timeslot: bookingData.timeslot.timeslot }),
              ...
              onDelete={async () => {
                ...
                const bookingDetails = bookedSeatsMap[selectedDate]?.[seatLabel]?.[showBooking.bookingDetails?.Timeslot];
                            ...
                            {isCreator && range.booking && (
                              <span style={{ display: 'flex', gap: 8 }}>
                                <button
                                  ...
                                  onClick={async (e) => {
                                    e.stopPropagation();
                                    // Open edit modal after refreshing backend bookings to avoid overwriting slots
                                    await openBookingModal({
                                      seatId: viewBookingDetails.seatId,
                                      seatLabel: viewBookingDetails.seatLabel,
                                      date: selectedDate,
